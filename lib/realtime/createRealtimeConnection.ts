...
export type ConnectionCallbacks = {
  ...
  transcript: (message: Message) => void;
  transfer: (summary: Transfer) => void;
  audioBlocked: () => void;
};
...
export function createRealtimeConnection(
  ...
  const handleEvent = (event: Record<string, unknown>) => {
    ...
    if (event.type === 'input_audio_buffer.speech_started')
      callbacks.status('listening'); // Server VAD cancels and truncates WebRTC playout automatically.
    if (event.type === 'output_audio_buffer.started') {
      playing = true;
      if (endingTimer) {
        clearTimeout(endingTimer);
        timers.delete(endingTimer);
        endingTimer = undefined;
      }
      callbacks.status('assistant speaking');
    }
    if (
      ...
      playing = false;
      if (ending) finish();
      else callbacks.status('listening');
    }
    if (
    ...
    if (event.type === 'response.function_call_arguments.done') {
      ...
      if (
        ...
        return;
      if (
        event.name === 'request_human_transfer' &&
        typeof args.reason === 'string' &&
        typeof args.customerSummary === 'string'
      ) {
        callbacks.transfer({
          reason: redact(args.reason),
          customerSummary: redact(args.customerSummary),
        });
        send({
          ...
          item: {
            ...
            output: JSON.stringify({ status: 'follow_up_request_ready' }),
          },
        });
        send({ type: 'response.create' });
      }
      if (event.name === 'end_call' && !ending) {
        ending = true;
        send({
          type: 'conversation.item.create',
          item: {
        ...
        if (!playing) endingTimer = delay(finish, 1600);
        delay(finish, 20000);
      }
    }
    if (event.type === 'error')
      fail('The voice session encountered a problem. Please try again.');
  };
  async function start() {
    ...
    if (
      ...
      fail('Voice requires a supported browser and a secure connection.');
      return;
    }
    ...
    try {
      ...
      const connectTimer = delay(
        () => fail('The voice assistant couldn’t connect. Please try again.'),
        35000,
      );
      pc.ontrack = (e) => {
        if (audio) {
          ...
          void audio.play().catch(() => {
            if (!closed) callbacks.audioBlocked();
          });
        }
      };
      ...
      dc.onmessage = (e) => {
        try {
          ...
          /* Ignore malformed events without logging customer data. */
        }
      };
      dc.onerror = () =>
        fail('The voice connection was interrupted. Please try again.');
      dc.onclose = () => {
        if (!closed) fail('The voice connection ended. Please try again.');
      };
      dc.onopen = () => {
      ...
      const response = await fetch('/api/realtime/session', {
        ...
        body: JSON.stringify({ sdp: offer.sdp, handoffContext: context, mode }),
        signal: abort.signal,
      });
      if (!response.ok) {
        let message = 'The voice assistant couldn’t connect. Please try again.';
        try {
      ...
      fail(
        error instanceof DOMException &&
          ...
          ? 'Microphone access is required for voice support.'
