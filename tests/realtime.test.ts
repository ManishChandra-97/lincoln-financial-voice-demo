...
void test('late microphone permission after close immediately stops capture',async()=>{
 let resolve!:(v:MediaStream)=>void,stopped=0;
 Object.defineProperty(globalThis,'navigator',{value:{mediaDevices:{getUserMedia:()=>new Promise<MediaStream>(r=>{resolve=r;})}},configurable:true});
 Object.defineProperty(globalThis,'RTCPeerConnection',{value:class{},configurable:true});
 const c=createRealtimeConnection(initialState().context,'phone-demo',callbacks());const pending=c.start();c.close();resolve({getTracks:()=>[{stop(){stopped++;}}]} as unknown as MediaStream);await pending;assert.equal(stopped,1);
});
...
void test('starting twice requests one microphone; a closed connection never requests access',async()=>{
 let requests=0,stopped=0,resolve!:(value:MediaStream)=>void;
 Object.defineProperty(globalThis,'navigator',{value:{mediaDevices:{getUserMedia:()=>{requests++;return new Promise<MediaStream>(r=>{resolve=r;});}}},configurable:true});
 const c=createRealtimeConnection(initialState().context,'phone-demo',callbacks());
 const pending=c.start();await c.start();assert.equal(requests,1);c.close();
 resolve({getTracks:()=>[{stop(){stopped++;}}]} as unknown as MediaStream);await pending;
 await c.start();assert.equal(requests,1);assert.equal(stopped,1);
 const unused=createRealtimeConnection(initialState().context,'phone-demo',callbacks());unused.close();await unused.start();assert.equal(requests,1);
});

void test('delayed closing audio finishes before capture stops and malformed tool events are ignored',async(t)=>{
 t.mock.timers.enable({apis:['setTimeout']});
 let stopped=0,transfers=0;const statuses:string[]=[];
 const track={enabled:true,stop(){stopped++;}};
 const dc={readyState:'open',onopen:null as null|(()=>void),onmessage:null as null|((e:{data:string})=>void),onclose:null,onerror:null,send:()=>{},close:()=>{}};
 const pc={ontrack:null,onconnectionstatechange:null,addTrack:()=>{},createDataChannel:()=>dc,createOffer:async()=>({sdp:'v=0\nm=audio'}),setLocalDescription:async()=>{},setRemoteDescription:async()=>{dc.onopen?.();},close:()=>{}};
 Object.defineProperty(globalThis,'navigator',{value:{mediaDevices:{getUserMedia:async()=>({getTracks:()=>[track],getAudioTracks:()=>[track]})}},configurable:true});
 Object.defineProperty(globalThis,'RTCPeerConnection',{value:function(){return pc;},configurable:true});
 Object.defineProperty(globalThis,'document',{value:{createElement:()=>({setAttribute:()=>{},play:async()=>{},pause:()=>{},remove:()=>{}})},configurable:true});
 const original=globalThis.fetch;globalThis.fetch=async()=>new Response('v=0\nm=audio');
 const c=createRealtimeConnection(initialState().context,'phone-demo',{...callbacks(),status:s=>statuses.push(s),transfer:()=>{transfers++;}});
 const event=(value:unknown)=>dc.onmessage?.({data:JSON.stringify(value)});
 try{
  await c.start();event(null);event([]);
  event({type:'response.function_call_arguments.done',name:'request_human_transfer',arguments:'null',call_id:'bad'});
  event({type:'response.function_call_arguments.done',name:'request_human_transfer',arguments:'{"reason":"help","customerSummary":"help"}'});
  assert.equal(transfers,0);
  event({type:'response.function_call_arguments.done',name:'end_call',arguments:'{}',call_id:'end'});
  t.mock.timers.tick(1000);event({type:'output_audio_buffer.started'});t.mock.timers.tick(2000);
  assert.equal(stopped,0);assert.equal(statuses.at(-1),'assistant speaking');
  event({type:'output_audio_buffer.stopped'});assert.equal(stopped,1);assert.equal(statuses.at(-1),'ended');
 }finally{c.close();globalThis.fetch=original;}
});
